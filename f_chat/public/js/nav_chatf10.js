...
function check_chat_enabled() {
    ...
    frappe.call({
        method: "f_chat.f_chat.doctype.chat_settings.chat_settings.is_chat_enabled",
...
function edit_chat_room(room_id, event) {
    ...
    frappe.call({
        method: "f_chat.get_room_details",
...
function setup_member_management_handlers(dialog, roomId) {
    ...
    dialog.$wrapper.off('change', '.member-role-select').on('change', '.member-role-select', function() {
        ...
        frappe.call({
            method: 'f_chat.update_member_role',
            args: {
                room_id: roomId,
                user_id: user,
                new_role: newRole
    ...
    dialog.$wrapper.off('click', '.remove-member-btn').on('click', '.remove-member-btn', function(e) {
        ...
        frappe.confirm(
            ...
            () => {
                frappe.call({
                    method: 'f_chat.remove_room_member',
                    args: {
                        room_id: roomId,
                        user_id: user
                    },
        ...
        fields: [
            ...
            },
            {
                ...
                get_data: function(txt) {
                    ...
                    return frappe.call({
                        method: 'f_chat.search_users_for_room',
                        args: {
                            search_term: txt || '',
                            room_id: roomId,
...
function update_chat_room_with_error_handling(room_id, values, dialog) {
    ...
    frappe.call({
        method: 'f_chat.update_room_settings',
        args: {
            room_id: room_id,
            settings: {
...
function show_cron_logs() {
    frappe.call({
        method: 'f_chat.f_chat.doctype.chat_settings.chat_settings.get_cron_logs',
        callback: function(r) {
            if (r.message && r.message.success) {
...
function check_for_new_messages_enhanced() {
    ...
    frappe.call({
        method: "f_chat.get_user_chat_status",
        callback: function(response) {
            if (response.message && response.message.success) {
        ...
        error: function(err) {
            ...
            setTimeout(() => {
                frappe.call({
                    method: "f_chat.get_user_chat_rooms",
...
function create_enhanced_room() {
    ...
    frappe.call({
        method: "f_chat.create_chat_room",
        args: {
            room_name: roomName,
            room_type: roomType,
...
function search_users_for_room_with_roles(searchTerm, roomId, resultsField, selectedUsers) {
    frappe.call({
        method: 'f_chat.search_users_for_chat_room',
        args: {
...
function add_users_with_roles_to_room(roomId, selectedUsers, dialog) {
    ...
    function addNextUser(index) {
        ...
        frappe.call({
            method: 'f_chat.add_member_to_room',
            args: {
                room_id: roomId,
...
function add_selected_users_to_room(roomId, selectedUsers, dialog) {
    ...
    frappe.call({
        method: 'f_chat.add_multiple_members_to_room',
        args: {
            room_id: roomId,
...
function send_enhanced_message() {
    ...
    const apiMethod = editingMessage ? "f_chat.edit_message" : "f_chat.send_message";
...
function mark_enhanced_room_as_read(roomId) {
    frappe.call({
        method: "f_chat.mark_room_as_read",
        args: { room_id: roomId },
        callback: function() {
            setTimeout(() => {
                check_for_new_messages_enhanced();
